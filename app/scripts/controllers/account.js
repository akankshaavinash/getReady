...
calendarDemoApp.constant('uiCalendarConfig', { calendars: {} })
    .controller('AccountCtrl', function($scope, $compile, $timeout, uiCalendarConfig, $uibModal, $rootScope, seeker, userService) {
        ...
        $scope.getInterviewSetup = function() {
            seeker.getInterviewSetup().then(function(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    var interviewTime = new Date(data[i].date);
                    var interviewTimeInMilliseconds = interviewTime.getTime();
                    
                    var currentDate = new Date()
                    var currentDateInMilliseconds = currentDate.getTime();
                   
                    if (currentDateInMilliseconds > interviewTimeInMilliseconds) {
                      // console.log('interview taken..');
                      data[i].interviewTaken = true;
                    }else{
                      // console.log('interview not taken');
                      data[i].interviewTaken = false;
                    }
        ...
        $scope.saveFeedback = function(feedback, jobseeker_email_id){
          feedback.jobseeker_email_id = jobseeker_email_id;
          feedback.interviewer_email_id = loginSessionData.currentUser.email;
          console.log(feedback);
          seeker.setInterviewerFeedback(feedback).then(function(data){
